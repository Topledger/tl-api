...
export default function ResearchPage() {
  ...
  // Research APIs are all APIs except Trading and Helium
  const availableApis = (apiData?.apis || []).filter(api => 
    api.menuName !== 'Trading' && api.menuName !== 'Helium'
  );
  ...
  const projectOptions = [
    ...
    ...projectNames.map(name => ({ value: name, label: name.charAt(0).toUpperCase() + name.slice(1) }))
  ];
