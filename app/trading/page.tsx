...
export default function TradingPage() {
  ...
  // Trading APIs are filtered by menuName === 'Trading'
  const availableApis = (apiData?.apis || []).filter(api => api.menuName === 'Trading');
          ...
          <div className="space-y-4">
            <div className="flex flex-col lg:flex-row lg:items-center gap-4">
              <div className="relative flex-1 lg:max-w-none">
                ...
                <input
                  type="text"
                  placeholder="Search Trading APIs by title, description..."
                  value={searchTerm}
                  onChange={(e) => {
        ...
        <div className="space-y-4">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center min-h-[60vh]">
              ...
              <p className="mt-2 text-gray-500">Loading Trading APIs...</p>
            </div>
          ) : filteredApis.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-gray-500">
                {apiData ? 'No Trading APIs found matching your search.' : 'Failed to load Trading APIs.'}
              </p>
            </div>
          ) : (
